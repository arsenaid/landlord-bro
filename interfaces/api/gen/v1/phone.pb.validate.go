...
func (m *PhoneNumber) validate(all bool) error {
	...
	if !_PhoneNumber_CountryCode_Pattern.MatchString(m.GetCountryCode()) {
		err := PhoneNumberValidationError{
			field:  "CountryCode",
			reason: "value does not match regex pattern \"^\\\\+?[1-9]\\\\d{1,4}$\"",
		}
		if !all {
			return err
		}
	...
	if !_PhoneNumber_Number_Pattern.MatchString(m.GetNumber()) {
		err := PhoneNumberValidationError{
			field:  "Number",
...
var _PhoneNumber_CountryCode_Pattern = regexp.MustCompile("^\\+?[1-9]\\d{1,4}$")

var _PhoneNumber_Number_Pattern = regexp.MustCompile("^[0-9]*$")
